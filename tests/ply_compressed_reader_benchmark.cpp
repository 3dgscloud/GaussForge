...
// Create realistic compressed PLY test data (simulating real Gaussian Splatting
// data)
gf::GaussianCloudIR CreateRealisticCompressedPLYData(int numPoints) {
  gf::GaussianCloudIR ir;
  ir.numPoints = numPoints;
  ir.meta.shDegree = 3; // Real PLY files typically use SH degree 3
...
// Compressed PLY read speed benchmark
static void BM_PlyCompressedReader_Read(benchmark::State &state) {
  const int numPoints = static_cast<int>(state.range(0));

  // Prepare test data (outside loop to avoid affecting benchmark)
  auto writer = gf::MakePlyCompressedWriter();
  auto reader = gf::MakePlyCompressedReader();

  auto testIR = CreateRealisticCompressedPLYData(numPoints);
  ...
  const auto &compressedPlyData = writeResult.value();
  const size_t dataSize = compressedPlyData.size();

  // Run benchmark
  for (auto _ : state) {
    auto result =
        reader->Read(compressedPlyData.data(), compressedPlyData.size(), {});
    if (!result.ok()) {
...
// Register benchmark: Test read speed for different compressed PLY file sizes
// Using common point count ranges: 1K, 10K, 100K, 500K, 1M, 5M
BENCHMARK(BM_PlyCompressedReader_Read)
    ->Arg(1000)    // 1K points
