...
// Create realistic spz test data (simulating real Gaussian Splatting data)
// .spz format supports SH degrees 0-3, we test with SH degree 3
gf::GaussianCloudIR CreateRealisticSpzData(int numPoints) {
  gf::GaussianCloudIR ir;
  ir.numPoints = numPoints;
  ir.meta.shDegree = 3; // Real spz files typically use SH degree 3
...
// Spz read speed benchmark
static void BM_SpzReader_Read(benchmark::State &state) {
  const int numPoints = static_cast<int>(state.range(0));

  // Prepare test data (outside loop to avoid affecting benchmark)
  auto writer = gf::MakeSpzWriter();
  auto reader = gf::MakeSpzReader();

  auto testIR = CreateRealisticSpzData(numPoints);
  ...
  const auto &spzData = writeResult.value();
  const size_t dataSize = spzData.size();

  // Run benchmark
  for (auto _ : state) {
    auto result = reader->Read(spzData.data(), spzData.size(), {});
    if (!result.ok()) {
...
// Register benchmark: Test read speed for different spz file sizes
// Using common point count ranges: 1K, 10K, 100K, 500K, 1M, 5M
BENCHMARK(BM_SpzReader_Read)
    ->Arg(1000)    // 1K points
