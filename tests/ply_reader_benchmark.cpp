...
// Create realistic PLY test data (simulating real Gaussian Splatting data)
gf::GaussianCloudIR CreateRealisticPLYData(int numPoints) {
...
// PLY read speed benchmark
static void BM_PlyReader_Read(benchmark::State &state) {
  const int numPoints = static_cast<int>(state.range(0));

  // Prepare test data (outside loop to avoid affecting benchmark)
  auto writer = gf::MakePlyWriter();
  auto reader = gf::MakePlyReader();

  auto testIR = CreateRealisticPLYData(numPoints);
  ...
  const auto &plyData = writeResult.value();
  const size_t dataSize = plyData.size();

  // Run benchmark
  for (auto _ : state) {
    auto result = reader->Read(plyData.data(), plyData.size(), {});
    if (!result.ok()) {
...
// Register benchmark: Test read speed for different PLY file sizes
// Using common point count ranges: 1K, 10K, 100K, 500K, 1M, 5M
BENCHMARK(BM_PlyReader_Read)
    ->Arg(1000)    // 1K points
