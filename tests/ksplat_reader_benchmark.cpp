...
// Ksplat read speed benchmark
static void BM_KsplatReader_Read(benchmark::State &state) {
  ...
  auto reader = gf::MakeKsplatReader();
  ...
  const auto &ksplatData = writeResult.value();
  const size_t dataSize = ksplatData.size();

  // Run benchmark
  for (auto _ : state) {
    auto result = reader->Read(ksplatData.data(), ksplatData.size(), {});
    if (!result.ok()) {
...
// Register benchmark: Test read speed for different ksplat file sizes
// Using common point count ranges: 1K, 10K, 100K, 500K, 1M, 5M
BENCHMARK(BM_KsplatReader_Read)
    ->Arg(1000)    // 1K points
